...
'''
 ...
 Observations:
 100,000 doesn't generate anything that seems to work or be worth tuning.
 500,000 and it starts to approach the pad but bounces off.
1000,000 not any better than previous.
'''
...
render_env = gym.make(PROBLEM_NAME, render_mode='human')
terminated = False
truncated = False

'''
render env separates rendering from the trained model to mitigate the risks of
all the wrappers doing something unexpected to rendering.
'''
...
for i in range(EVAL_ENV_INTERACTION_STEPS):
    ...
    # Execute the same action in the rendering environment
    render_env.step(single_action)
...
print(f"Training and evaluation finished. Total runtime: {total_runtime:.2f} seconds")
